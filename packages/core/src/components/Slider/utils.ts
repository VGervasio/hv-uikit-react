...
): number => Math.floor((scaledValue - minPointValue) * inverseStepValue);
...
export const createMark = (
  ...
  if (markProperties.length > 0) {
    ...
    for (let index = 0; index <= divisionQuantity; index += roundedMarkStep) {
      let labelValue: number | string | React.ReactNode =
        knobsPositionToScaledValue(index, minPointValue, stepValue).toFixed(
          markDigits
        );
...
export const generateDefaultKnobProperties = (
  ...
  const defaultKnobStyles = {
    ...
    trackColor: theme.colors.acce1,
    dragColor: theme.colors.acce1,
    knobRingColor: theme.colors.atmo1,
  };

  const disabledKnobStyles = {
    color: theme.colors.atmo5,
    hoverColor: theme.colors.atmo5,
    trackColor: theme.colors.atmo5,
    dragColor: theme.colors.atmo5,
    knobRingColor: theme.colors.atmo5,
  };
...
export const convertStatusToArray = (
  ...
  arrayDefaultStatus?: any[]; // TODO: REVIEW
} => {
  const result: {
    arrayStatus?: HvFormStatus[];
    arrayDefaultStatus?: any[]; // TODO: REVIEW
