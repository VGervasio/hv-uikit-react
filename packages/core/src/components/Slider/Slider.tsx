...
import { HvFormStatus, HvLabel } from "../Forms";
...
export interface HvMarkProperty {
  position: number; // TODO: REVIEW NOT REQUIRED
  label?: string;
}

export type HvSliderProps = HvBaseProps<HTMLDivElement, { onChange }> & {
  /**
   * The slider name.
   */
  name?: string;
   ...
   */
  label?: string; // TODO: REVIEW REACT NODE
  /**
...
export const HvSlider = ({
  ...
  const performValidation = useCallback(() => {
    ...
    const newValidationState: keyof typeof validationStates =
      knobsPositions.map((position) => {
  ...
  const onMouseDownHandler = (event: React.MouseEvent) => {
    if (
      (
        event.target as React.MouseEvent["target"] & { className: string }
      ).className // TODO: REVIEW
        .includes("track")
    ) {
  ...
  return (
    ...
    >
      ...
      >
        {isSingle && (
          <StyledSlider
            //id={setId(elementId, "slider")} // TODO: DOES NOT EXIST
